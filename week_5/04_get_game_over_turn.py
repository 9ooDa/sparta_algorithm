...
def get_game_over_turn_count(horse_count, game_map, horse_location_and_directions):
    n = len(game_map)
    # chess_map size: n x n, most inner bracket is for the linked list to represent the horses
    current_stacked_horse_map = [[[] for _ in range(n)] for _ in range(n)]

    for i in range(horse_count):
        r, c, d = horse_location_and_directions[i]
        current_stacked_horse_map[r][c].append(i)  # ith horse is on (r, c)
    ...
    while turn_count <= 1000:
        for horse_index in range(horse_count):
            ...
            new_r = r + dr[d]  # 현재 위치 + 방향
            new_c = c + dc[d]

            if not 0 <= new_r < n or not 0 <= new_c < n or game_map[new_r][new_c] == 2:  # 파란색이거나 맥을 나갔을 때 reverse
            ...
            for i in range(len(current_stacked_horse_map[r][c])):
                ...
                if horse_index == current_stacked_horse_index:
                    moving_horse_index_array = current_stacked_horse_map[r][c][i:]  # 위에 있는 말이랑 같이 이동
                    current_stacked_horse_map[r][c] = current_stacked_horse_map[r][c][:i]  # 원래 있던 위치에 남은 말
                    break
